...
export function override() {
    global.vertical_overview.GSFunctions['WorkspacesView'] = Util.overrideProto(
        WorkspacesView.WorkspacesView.prototype, WorkspacesViewOverride);

    if (WorkspacesView.SecondaryMonitorDisplay) {
        global.vertical_overview.GSFunctions['SecondaryMonitorDisplay'] = Util.overrideProto(
            WorkspacesView.SecondaryMonitorDisplay.prototype, SecondaryMonitorDisplayOverride);
    }

    if (global.vertical_overview.default_old_style_enabled) {
        Main.overview._overview._controls._workspacesDisplay
            .add_style_class_name('vertical-overview');
    }
}
...
export function reset() {
    ...
    if (global.vertical_overview.default_old_style_enabled) {
        Main.overview._overview._controls._workspacesDisplay
            .remove_style_class_name('vertical-overview');
    }
}
...
var WorkspacesViewOverride = {
    ...
    _getSpacing(box, fitMode, _vertical) {
        ...
        const totalHeight = global.screen_height;
        const availableSpace = (totalHeight - workspaceHeight) / 2 -
            (global.vertical_overview.workspacePeek || 0);
...
var SecondaryMonitorDisplayOverride = {
    ...
    vfunc_allocate(box) {
        ...
        const scale =
            Main.layoutManager.getWorkAreaForMonitor(this._monitorIndex).width /
            Main.layoutManager.primaryMonitor.width;
        const leftOffset =
            Main.overview._overview._controls.layoutManager.leftOffset * scale;
        const rightOffset =
            Main.overview._overview._controls.layoutManager.rightOffset * scale;
